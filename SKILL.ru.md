...
**Безопасность при нескольких параллельных сессиях:** `ask.py` и `check_new.py` читают через
`getUpdates?offset=0` и хранят "что уже видели" локально в `.last_offset_<session_id>.json`. `getUpdates` у
Telegram — API для ОДНОГО потребителя: любой вызов с `offset > 0` подтверждает ("забывает") на сервере все
более ранние update'ы для ВСЕХ клиентов токена, не только для вызывающего. Если несколько параллельных
сессий делят одного бота (обычная ситуация — у каждой свой `session_id`), наивный positive offset от одной
сессии молча стёр бы сообщение, адресованное другой.

Но и `offset=0` сам по себе имеет ловушку, которая укусила на практике: раз никто не двигает offset,
серверная очередь **растёт без границы** (Telegram держит update'ы 24ч), а `getUpdates` режет каждый ответ
по `limit`. Как только backlog превысил `limit`, `offset=0` отдаёт только *самые старые* `limit` штук —
новейшие сообщения выпадают за окно и становятся **невидимы всем сессиям**, которые дружно рапортуют
`NOTHING_NEW`, пока пользователь кричит в пустоту. Два предохранителя чинят это, не возвращая взаимного
стирания:
1. **`limit=100`** (максимум Telegram) — самое широкое доступное окно.
2. **Ограниченный drain** (`check_new.py`): когда очередь близка к переполнению (`>= DRAIN_WHEN_QUEUE_OVER`),
   сдвигаем offset до `safe_drain_watermark` = **минимум** `last_update_id` среди сессий, чей
   `.last_offset_*.json` трогался в пределах `ACTIVE_SESSION_WINDOW_SEC`. Раз это минимум по *активным*
   сессиям — всё ниже него уже видели все они, так что сброс не стирает ничего непрочитанного; устаревшие
   (мёртвые) сессии исключаются, чтобы не заклинить drain навсегда.

Если добавляешь новый скрипт с `getUpdates` — следуй тем же правилам: `offset=0` + `limit=100` для чтения, и
продвигать offset только через watermark-минимум по активным сессиям.
...
**Ack при взятии в работу.** Когда задача приходит через Telegram (поймана `check_new.py`) и на неё уйдёт
больше, чем мгновенный тривиальный ответ — сразу, **до начала работы**, отправить короткий `notify.py`-ack,
например "📥 Прочитал, взял в работу: <одна строка о том, что делаешь>". Зачем: пока ты погружён в
длительную задачу, пользователь (который не за компом, смотрит в Telegram) иначе не видит ничего и не
понимает, взята ли задача вообще. Одного ack при взятии достаточно — **не** спамить апдейтами по ходу
работы; следующее нужное сообщение — completion-нотификация по завершении. (Мгновенный тривиальный ответ
ack не требует — просто ответь.)
